...
class Hed_R2(nn.Module):
    def __init__(self, input_nc, output_nc, ngf):
        ...
        self.conv3_s = nn.Conv2d(ngf * 12, output_nc, 1, 1, 0)
        self.conv4_s = nn.Conv2d(ngf * 16, output_nc, 1, 1, 0)
    ...
    def forward(self, input):
        ...
        s3 = torch.cat((up4, e3_3), 1)
        s3_1 = self.conv3_s(s3)
        s3_2 = self.upsample_4(s3_1)
        side_output3 = self.Sigmoid(s3_2)

        s4 = torch.cat((up5, e4_3), 1)
        s4_1 = self.conv4_s(s4)
        ...
        s3_R = torch.cat((up4_R, e3_3_R), 1)
        s3_1_R = self.conv3_s(s3_R)
        s3_2_R = self.upsample_4(s3_1_R)
        side_output3_R = self.Sigmoid(s3_2_R)

        s4_R = torch.cat((up5_R, e4_3_R), 1)
        s4_1_R = self.conv4_s(s4_R)
        ...
        # e1_1_R2 = e1_1_R2 + e1_1_R
        e1_2_R2 = self.batch_norm(self.relu(self.conv1_2(e1_1_R2)))
        p1_R2 = self.maxpool(e1_2_R2) 

        e2_1_R2 = self.batch_norm2(self.relu(self.conv2_1(p1_R2)))
        # e2_1_R2 = e2_1_R2 + e2_1_R
        e2_2_R2 = self.batch_norm2(self.relu(self.conv2_2(e2_1_R2)))
        p2_R2 = self.maxpool(e2_2_R2) 

        def23_R2 = self.convD_23(p2_R2)
        e3_1_R2 = self.batch_norm4(self.relu(self.conv3_1(def23_R2)))
        # e3_1_R2 = e3_1_R2 + e3_1_R
        e3_2_R2 = self.batch_norm4(self.relu(self.conv3_2(e3_1_R2)))
        e3_3_R2 = self.batch_norm4(self.relu(self.conv3_3(e3_2_R2)))
        p3_R2 = self.maxpool(e3_3_R2) 

        def34_R2 = self.convD_34(p3_R2)
        e4_1_R2 = self.batch_norm8(self.relu(self.conv4_1(def34_R2)))
        # e4_1_R2 = e4_1_R2 + e4_1_R
        e4_2_R2 = self.batch_norm8(self.relu(self.conv4_2(e4_1_R2)))
        e4_3_R2 = self.batch_norm8(self.relu(self.conv4_3(e4_2_R2))) 
        p4_R2 = self.maxpool(e4_3_R2) 
        up4_R2 = self.upsample_2(e4_3_R2)

        def45_R2 = self.convD_45(p4_R2)
        e5_1_R2 = self.batch_norm8(self.relu(self.conv5_1(def45_R2)))
        # e5_1_R2 = e5_1_R2 + e5_1_R
        e5_2_R2 = self.batch_norm8(self.relu(self.conv5_2(e5_1_R2))) 
        e5_3_R2 = self.batch_norm8(self.relu(self.conv5_2(e5_2_R2)))
        up5_R2 = self.upsample_2(e5_3_R2)

        s1_1_R2 = self.conv1_s(e1_2_R2)
        side_output1_R2 = self.Sigmoid(s1_1_R2)

        s2_1_R2 = self.conv2_s(e2_2_R2)
        s2_2_R2 = self.upsample_2(s2_1_R2)
        side_output2_R2 = self.Sigmoid(s2_2_R2)

        s3_R2 = torch.cat((up4_R2, e3_3_R2), 1)
        s3_1_R2 = self.conv3_s(s3_R2)
        s3_2_R2 = self.upsample_4(s3_1_R2)
        side_output3_R2 = self.Sigmoid(s3_2_R2)

        s4_R2 = torch.cat((up5_R2, e4_3_R2), 1)
        s4_1_R2 = self.conv4_s(s4_R2)
        s4_2_R2 = self.upsample_8(s4_1_R2)
        side_output4_R2 = self.Sigmoid(s4_2_R2)

        s5_1_R2 = self.conv5_s(e5_3_R2)
