...
class ISBI(data.Dataset):
    def __init__(self,dataPath='/media/data1/JY/neuron/ISBI/ISBI_train/train_with_def_20',loadSize=512):
        super(ISBI, self).__init__()
        # list all images into a list
        self.image_list = [x for x in listdir(dataPath + '/img/') if is_image_file(x)]
        self.dataPath = dataPath
        self.loadSize = loadSize

    def __getitem__(self, index):
        # 1. Read one data from file (e.g. using numpy.fromfile, PIL.Image.open).
        img_path = os.path.join(self.dataPath,'img',self.image_list[index])
        label_path = os.path.join(self.dataPath,'gt',self.image_list[index])

        img = default_loader(img_path) 
        label = default_loader_label(label_path)
        img = ToTensor(img)
...
class ISBI_TEST(data.Dataset):
    def __init__(self,dataPath='/media/data1/JY/neuron/ISBI/test/imgs/',loadSize=512):
