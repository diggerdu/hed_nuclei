...
def th_batch_map_coordinates(input, coords, order=1):
    ...
    input_size_h = input.size(1)
    input_size_w = input.size(2)
    n_coords = coords.size(1)
    coords_clone = coords.clone()

    # coords = torch.clamp(coords, 0, input_size - 1)
    coords_clone[..., 0] = torch.clamp(coords[..., 0], 0, input_size_h - 1)
    coords_clone[..., 1] = torch.clamp(coords[..., 1], 0, input_size_w - 1)
    coords = coords_clone
    coords_lt = coords.floor().long()
    coords_rb = coords.ceil().long()
...
def th_generate_grid(batch_size, input_size_h, input_size_w, dtype, cuda):
    grid = np.meshgrid(
        range(input_size_h), range(input_size_w), indexing='ij'
    )
...
def th_batch_map_offsets(input, offsets, grid=None, order=1):
    ...
    if grid is None:
        grid = th_generate_grid(batch_size, input_size_h,input_size_w, offsets.data.type(), offsets.data.is_cuda)
